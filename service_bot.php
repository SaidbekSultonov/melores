<?php
    ...
    const TOKEN = '1931399705:AAGipbWOhynFHLeHFyfgEFFBGc6K3TcPrL8';
    ...
    function deleteMessageUser($conn, $chat_id){
        $selectMessageId = "SELECT * FROM service_message_id WHERE chat_id = ".$chat_id;
        ...
        if (pg_num_rows($resultMessageId) > 0){
            ...
            $deleteMessageId = "DELETE FROM service_message_id WHERE chat_id = ".$chat_id;
    ...
    function insertMessageId($response,$conn, $chat_id){
        ...
        $insertMessageId = "INSERT INTO service_message_id (chat_id,message_id) VALUES (".$chat_id.",".$message_id.")";
        $resultMessageId = pg_query($conn, $insertMessageId);
    }

    function insertStatistics($section,$button_id,$chat_id,$conn){
        $selectClick = "SELECT * FROM service_statistics WHERE type = '".$section."' AND button_id = ".$button_id." AND chat_id = ".$chat_id;
        ...
        if (pg_num_rows($resultClick) > 0){
            ...
            $updateClick = "UPDATE service_statistics SET click_count = ".$click_count." WHERE type = '".$section."' AND button_id = ".$button_id." AND chat_id = ".$chat_id;
            $resultUpdateClick = pg_query($conn, $updateClick);
        } else {
            $addClickUser = "INSERT INTO service_statistics (chat_id, type, button_id, click_count) VALUES (".$chat_id.", '".$section."', ".$button_id.", 1)";
    ...
    $menu = json_encode([
        'inline_keyboard' => [
            [
                ['text' => "🔀 Yo'nalishlar",'callback_data' => 'directions'],
    ...
    if ($text == "/start") {
        $checkUser = "SELECT * FROM service_users WHERE chat_id = ".$chat_id;
        $resultCheck = pg_query($conn, $checkUser);
        if (!pg_num_rows($resultCheck) > 0) {
            $addUser = "INSERT INTO service_users (chat_id,username) VALUES (".$chat_id.",'".$user_name."')";
            $resultAddUser = pg_query($conn, $addUser);
            $addStep = "INSERT INTO service_step (chat_id,step_1, step_2) VALUES (".$chat_id.",0,0)";
            $resultStep = pg_query($conn, $addStep);
            $addLastId = "INSERT INTO service_last_id (chat_id,last_id) VALUES (".$chat_id.",0)";
            $resultLastId = pg_query($conn, $addLastId);
        } else {
            $stepUpdate = "UPDATE service_step SET step_1 = 0, step_2 = 0 WHERE chat_id = ".$chat_id;
            $resultStep = pg_query($conn, $stepUpdate);
            $updateLastId = "UPDATE service_last_id SET last_id = 0 WHERE chat_id = ".$chat_id;
    ...
    $sql = "SELECT * FROM service_step WHERE chat_id = ".$chat_id;
    ...
    $lastId = "SELECT * FROM service_last_id WHERE chat_id = ".$chat_id;
    ...
    if ($step_1 == 0 and $step_2 == 0 and $text != "/start"){
        ...
        if ($data == "uz"){
            $stepUpdate = "UPDATE service_step SET step_1 = 1 WHERE chat_id = ".$chat_id;
            $resultStep = pg_query($conn, $stepUpdate);
            ...
            $stepUpdate = "UPDATE service_step SET step_1 = 2 WHERE chat_id = ".$chat_id;
            $resultStep = pg_query($conn, $stepUpdate);
    ...
    if ($step_2 == 0 and $text != "/start"){
        if (isset($update->message->contact)){
            ...
            $stepUpdate = "UPDATE service_step SET step_2 = 1 WHERE chat_id = ".$chat_id;
            $resultStep = pg_query($conn, $stepUpdate);

            $addPhoneNumber = "UPDATE service_users SET phone_number = '".$number."' WHERE chat_id = ".$chat_id;
    ...
    if ($step_2 == 1 and $text != "/start"){
        deleteMessageUser($conn, $chat_id);
        if ($data == "directions"){
            $stepUpdate = "UPDATE service_step SET step_2 = 2 WHERE chat_id = ".$chat_id;
            $resultStep = pg_query($conn, $stepUpdate);

            $selectService = "SELECT * FROM services WHERE type = 1 ORDER BY order_column ASC";
            $resultService = pg_query($conn, $selectService);

            if ($step_1 == 1){
                $text_section = "Yo'nalishlardan birini tanlang ⤵️";
                $keyboard_back = "Ortga ↩️";
                $empty_section_text = "*Bu bo'limda ma'lumot mavjud emas ❗️*";
            }
            else if ($step_1 == 2){
                $text_section = "Выберите один из разделов ⤵️";
            ...
            $stepUpdate = "UPDATE service_step SET step_2 = 100 WHERE chat_id = ".$chat_id;
            $resultStep = pg_query($conn, $stepUpdate);

            $sql = 'SELECT * FROM service_last_id WHERE chat_id = '.$chat_id;
            $result = pg_query($conn, $sql);

            if (pg_num_rows($result) > 0) {
                $sqlLastId = "UPDATE service_last_id SET last_id = 0 WHERE chat_id = ".$chat_id;
                pg_query($conn, $sqlLastId);
            }
            else {
                $insertInto = "INSERT INTO service_last_id (chat_id, last_id) VALUES (".$chat_id.", 0)";
                pg_query($conn, $insertInto);
            }

            $selectVideo = "SELECT * FROM company_files WHERE status = 1 and type = 'video' and file_id is null";
    ...
    if ($step_2 == 2 and $text != "/start"){
        ...
        if ($service[0] == "service"){
            $data_service_id = $service[1];

            insertStatistics('service', $data_service_id, $chat_id, $conn);

            $clickService = "SELECT * FROM services WHERE id = ".$data_service_id;
            $resultClickService = pg_query($conn, $clickService);

            if ($resultClickService > 0){
                $row = pg_fetch_assoc($resultClickService);
                $click_count = $row['click_count'] + 1;
                $clickCountUpdate = "UPDATE services SET click_count = ".$click_count." WHERE id = ".$data_service_id;
                $resultUpdateClickCount = pg_query($conn, $clickCountUpdate);
            }

            $stepUpdate = "UPDATE service_step SET step_2 = 3 WHERE chat_id = ".$chat_id;
            $resultStep = pg_query($conn, $stepUpdate);

            $selectService = "SELECT * FROM services_types WHERE status = 1 AND services_id = ".$data_service_id." ORDER BY order_column ASC";
            $resultService = pg_query($conn, $selectService);

            if ($step_1 == 1){
                $keyboard_back = "Ortga ↩️";
                $text_service_type = "Bo'limlar birini tanlang ⤵️";
                $empty_section_text = "*Bu bo'limda ma'lumot mavjud emas❗️*";
            }
            else if ($step_1 == 2){
                $keyboard_back = "Назад ↩️";
                $text_service_type = "Выберите один из разделов ⤵️";
                $empty_section_text = "*В этом разделе нет информации ❗️*";
            }
    ...
    if ($step_2 == 3 and $text != "/start"){
        ...
        if ($serviceType[0] == "serviceType"){
            ...
            $clickService = "SELECT * FROM services_types WHERE id = ".$data_service_type_id;
            $resultClickService = pg_query($conn, $clickService);

            if ($resultClickService > 0){
                $row = pg_fetch_assoc($resultClickService);
                $click_count = $row['click_count'] + 1;
                $clickCountUpdate = "UPDATE services_types SET click_count = ".$click_count." WHERE id = ".$data_service_type_id;
                $resultUpdateClickCount = pg_query($conn, $clickCountUpdate);
            }

            $stepUpdate = "UPDATE service_step SET step_2 = 4 WHERE chat_id = ".$chat_id;
            $resultStep = pg_query($conn, $stepUpdate);
            ...
            $selectCompany = "
                SELECT 
                       ...
                       c.title_uz, 
                       c.title_ru 
                FROM company_services_type AS cst
                INNER JOIN company AS c ON cst.company_id = c.id
            ...
            if ($resultStep == true and pg_num_rows($resultService) > 0){
                ...
                if ($step_1 == 1){
                    $keyboard_back = "Ortga ↩️";
                    $text_company_type = "Yo'nalishlardan birini tanlang ⤵️";
                }
                else if ($step_1 == 2){
                    $keyboard_back = "Назад ↩️";
                    $text_company_type = "Выберите одно из направлений ⤵️";
                }
    ...
    if ($step_2 == 4 and $text != "/start"){
        ...
        if ($company[0] == "company"){
            ...
            $clickService = "SELECT * FROM company WHERE id = ".$data_company_id;
            $resultClickService = pg_query($conn, $clickService);

            if ($resultClickService > 0){
                $row = pg_fetch_assoc($resultClickService);
                $click_count = $row['click_count'] + 1;
                $clickCountUpdate = "UPDATE company SET click_count = ".$click_count." WHERE id = ".$data_company_id;
                $resultUpdateClickCount = pg_query($conn, $clickCountUpdate);
            }

            $stepUpdate = "UPDATE service_step SET step_2 = 5 WHERE chat_id = ".$chat_id;
            $resultStep = pg_query($conn, $stepUpdate);
            ...
            $selectDistrict = "
                ...
                INNER JOIN district AS td ON cf.district_id = td.id
            ...
            if ($resultStep == true and pg_num_rows($resultService) > 0){
                ...
                if ($step_1 == 1){
                    $keyboard_back = "Ortga ↩️";
                    $text_company_type = "Bo'limlardan birini tanlang ⤵️";
                }
                else if ($step_1 == 2){
                    $keyboard_back = "Назад ↩️";
                    $text_company_type = "Выберите один из разделов ⤵️";
                }
    ...
    if ($step_2 == 5 and $text != "/start"){
        ...
        if ($district[0] == "district"){
            ...
            $clickService = "SELECT * FROM district WHERE id = ".$data_district_id;
            $resultClickService = pg_query($conn, $clickService);

            if ($resultClickService > 0){
                $row = pg_fetch_assoc($resultClickService);
                $click_count = $row['click_count'] + 1;
                $clickCountUpdate = "UPDATE district SET click_count = ".$click_count." WHERE id = ".$data_district_id;
                $resultUpdateClickCount = pg_query($conn, $clickCountUpdate);
            }

            $stepUpdate = "UPDATE service_step SET step_2 = 6 WHERE chat_id = ".$chat_id;
            $resultStep = pg_query($conn, $stepUpdate);
    ...
    if ($step_2 == 6 and $text != "/start"){
        ...
        $back = explode("_", $data);
        if ($back[0] == "back"){
            $data_service_id = $back[1];
            $data_service_type_id = $back[2];
            $data_company_id = $back[3];
            ...
            if ($resultStep == true and pg_num_rows($resultDistrict) > 0){
                $k = 1;
                while($row = pg_fetch_assoc($resultDistrict)) {
                    $district_id = $row['id'];
                    if ($step_1 == 1){
    ...
    if ($step_2 == 100 and $text != "/start"){
        ...
        if ($explode_video[0] == "video"){
            deleteMessageUser($conn, $chat_id);
            $stepUpdate = "UPDATE service_step SET step_2 = 101 WHERE chat_id = ".$chat_id;
            $resultStep = pg_query($conn, $stepUpdate);

            $updateLastId = "UPDATE service_last_id SET last_id = ".$explode_video[1]." WHERE chat_id = ".$chat_id;
            $resultLastId = pg_query($conn, $updateLastId);
    ...
    if ($step_2 == 101 and $text != "/start"){
        if (isset($message->video)) {
            ...
            $updateFileId = "UPDATE company_files SET file_id = '".$file_id."' WHERE id = ".$last_id;
