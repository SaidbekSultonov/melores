...
$conn = pg_connect("host=localhost dbname=vkoenlqd_original_db user=vkoenlqd_original_user password=_(1[F#b@D{Sd");
    ...
    //   $penalties = UserPenalties::find()->all();
      ...
      $penalty_sql = "SELECT * FROM user_penalties";
      $penalty_result = pg_query($conn,$penalty_sql);
      
      while ($row_pen = pg_fetch_assoc($penalty_result)) {
          $sql_del = "DELETE FROM user_penalties WHERE id = ".$row_pen['id'];
          $del_result = pg_query($conn,$sql_del);
      }
      ...
      if (!empty($arr_pen)) {
        ...
        foreach ($arr_pen as $key2 => $value2) {
            $sqlTxt = "INSERT INTO otdel_log_message (`user_id`,`order_id`,`sum`,`delay_day_count`) VALUES (".$value2['order_id'].",".$value2['order_id'].",".$value2['sum'].",".$value2['day'].")";
          
          $resultInsert2 = pg_query($conn,$sqlTxt);
