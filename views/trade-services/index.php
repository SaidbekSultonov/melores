        ...
        <?php if (!empty($model)): ?>
            <?php foreach ($model as $key => $value): ?>
                <tr>
                    ...
                    <td>
                        <?php
                            if (isset($value->parent->title_uz)) {
                                echo $value->parent->title_uz;
                            } else {
                                echo "Kategoriyaga qo'shilmagan";
                            }
                        ?>
                    </td>
                    ...
                    <td>
                        ...
                        <a href="/index.php/trade-services/delete?id=<?php echo $value->id; ?>" id="delete-services" class="glyphicon glyphicon-trash"></a>         
                    </td>
                </tr>
            <?php endforeach; ?>
            <?php else: ?>
...
<script>
    ...
    $(function(){
        ...
        $('select').change('select2:select', function (e) {
            ...
            $.ajax({
                url: '/index.php/trade-services/order',
                dataType: 'json',
                type: 'GET',
                data: {
                    id: id,
    ...
    $(document).on('click','#open-modal-services',function(){
        $.ajax({
            url: '/index.php/trade-services/create',
            dataType: 'json',
            type: 'GET',
            success: function (response) {
                if (response.status == 'success') {
    ...
    $(document).on('click','.save-services',function(){
        ...
        let services_category_id = $('#category_id').val()

        $.ajax({
            url: '/index.php/trade-services/save-services',
            dataType: 'json',
            type: 'GET',
            data:{
    ...
    $(document).on('click','#update-services',function(){
        ...
        $.ajax({
            url: '/index.php/trade-services/update',
            dataType: 'json',
            type: 'GET',
            data: {id: id},
            success: function (response) {
    ...
    $(document).on('click','.update-save-services',function(){
        ...
        $.ajax({
            url: '/index.php/trade-services/update-save',
            dataType: 'json',
            type: 'GET',
            data:{
                services_uz: services_uz,
